...
from Bybit_Access._exceptions import FailedRequestError, InvalidRequestError
from . import VERSION # import from __init__.py
from Bybit_Access import _helpers
...
class _HTTPManager:
    def __init__(self, endpoint=None, api_key=None, api_secret=None,
        ...
        self.logger.debug("Initializing V2 HTTP session.")
        ...
        while True:
            ...
            # If requests fires an error, retry.
                ...
                if "order/create" in path:
                    return # bei order setzen & exception kam None zurückgeben
                elif self.force_retry:
            ...
            # exception handling wenn ret_code nicht in antwort ist (keyerror)
            try:
                s_json[ret_code]
            except Exception as e:
                self.logger.error(f"{type(e).__name__} - 'ret_code' not in s_json")
            ...
            if s_json[ret_code]:
                ...
                if s_json[ret_code] in self.retry_codes:
                    ...
                    # Log the error.
                    # self.logger.error(f"{error_msg}. {retries_remaining}")
                    self.logger.error(f"returning None and ignoring error: {error_msg}.")
                    return
